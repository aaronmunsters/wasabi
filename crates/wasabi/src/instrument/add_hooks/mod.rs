...
fn setup_instrument(function: &mut Function, ty: FunctionType, instrumented_body: &mut Vec<Instr>, instr: &Instr, location: &(Instr, Instr)) {
    let input_tmps = function.add_fresh_locals(ty.inputs());
    let result_tmps = function.add_fresh_locals(ty.results());
    ...
    restore_locals_with_i64_handling(instrumented_body, input_tmps.iter().chain( result_tmps.iter()).copied(), function);
}
