...
impl EncodeState {
    ...
    encode_state_idx_fns!(insert_element_idx, map_element_idx, element_idx, Element, "element");
...
fn encode_imports(module: &Module, state: &mut EncodeState) -> we::ImportSection {
    ...
    add_imports!(tables, insert_table_idx, Table, |t: &Table| get_tabletype_from_table(t));
...
fn encode_tables(
    ...
    for (hl_element_idx, element) in module.elements() {
        ...
        let expr = element
            .init
            .iter()
            .map(|instrs| -> Result<ConstExpr, EncodeError> {Ok(encode_single_instruction_with_end(instrs, state)?)})
