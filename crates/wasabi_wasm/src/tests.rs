...
const NAME_SECTION_TEST_BINARY: &str =
    "../../test-inputs/wasm-feature-tests/name-section/wabt-tests/names.wasm";
const BANANABREAD_REAL_WORLD_TEST_BINARY: &str =
    "../../test-inputs/real-world-binaries/bananabread/bb.wasm";
...
fn collect_all_function_types_in_test_set() {
    ...
    for_each_valid_wasm_binary_in_test_set(|path| {
        let (module, _, _) = Module::from_file(path).unwrap_or_else(|err| {
            panic!("Could not parse valid binary '{}': {err}", path.display())
        });
    ...
    fs::write(
        "../../test-outputs/collect-types/function_type_count.csv",
        output_contents,
    )
    .unwrap();

    let val_type_seq_count = DashMap::new();
    type_count.par_iter().for_each(|(func_ty, count)| {
    ...
    fs::write(
        "../../test-outputs/collect-types/val_type_seq_count.csv",
        output_contents,
    )
    .unwrap();
}
...
fn roundtrip_produces_same_module_ast() {
    for_each_valid_wasm_binary_in_test_set(|path| {
        let (module, _offsets, warnings) = Module::from_file(path).unwrap_or_else(|err| {
            panic!("Could not parse valid binary '{}': {err}", path.display())
        });
        ...
        let bytes = module.to_bytes().unwrap_or_else(|err| {
            panic!("Could not encode valid binary '{}': {err}", path.display())
        });
...
fn type_checking_valid_files() {
    for_each_valid_wasm_binary_in_test_set(|path| {
        ...
        TypeChecker::check_module(&module).unwrap_or_else(|_| {
            panic!(
                "Valid binary '{}' should type check, but did not",
                path.display()
            )
        });
    });
...
fn decode_encode_is_valid_wasm() {
    for_each_valid_wasm_binary_in_test_set(|path| {
        ...
        module.to_file(output_path).unwrap_or_else(|err| {
            panic!(
                "Could not encode valid binary to file '{}': {err}",
                output_path.display()
            )
        });

        wasm_validate(output_path).unwrap_or_else(|err| {
            panic!(
                "Written binary did not validate '{}': {err}",
                output_path.display()
            )
        });
    });
...
fn section_offsets_like_objdump() {
    ...
    assert_eq!(
        offsets.section_offsets(SectionId::Custom("name".to_string())),
        vec![0x1f]
    );
...
fn code_offsets_like_objdump() {
    ...
    // Expected values are taken from wasm-objdump output.
    assert_eq!(
        offsets.function_idx_to_offset(Idx::from(383u32)),
        Some(0x5522)
    );
    assert_eq!(
        offsets.function_offset_to_idx(0x5522),
        Some(Idx::from(383u32))
    );
    assert_eq!(
        offsets.function_idx_to_offset(Idx::from(384u32)),
        Some(0x5545)
    );
    assert_eq!(
        offsets.function_offset_to_idx(0x5545),
        Some(Idx::from(384u32))
    );

    assert_eq!(
        offsets.function_idx_to_offset(Idx::from(3641u32)),
        Some(0x1e38b7)
    );
    assert_eq!(
        offsets.function_offset_to_idx(0x1e38b7),
        Some(Idx::from(3641u32))
    );
    assert_eq!(
        offsets.function_idx_to_offset(Idx::from(3642u32)),
        Some(0x1e38d2)
    );
    assert_eq!(
        offsets.function_offset_to_idx(0x1e38d2),
        Some(Idx::from(3642u32))
    );
}
...
fn error_offsets_correct() {
    fn assert_error_offset(binary: Vec<u8>, expected_offset: usize) {
        ...
        assert!(
            result.is_err(),
            "Parsing did not fail, but should have because binary {binary:?} is invalid"
        );
    ...
    let valid_code_element = &[
        2,   // Size in bytes
        0,   // Local count
        0xb, // Body: only end instruction
    ]
    .as_ref();

    let section_size_too_short = [
        valid_wasm_header,
        &[
            10, // Section id: code
            2,  // WRONG Section size in bytes: should be 4 bit is 2
            1,  // Code elements
        ],
        valid_code_element,
    ]
    ...
    let code_element_size_too_short = [
        valid_wasm_header,
        &[
            10, 5, 1, // Code section, size in bytes, element count.
            2, // Code element 1; WRONG Size in bytes: should be 3.
            0, // Local count
            0x1, 0xb, // Body: nop, end
        ],
    ]
    .concat();
    ...
    let code_element_size_too_long = [
        valid_wasm_header,
        &[
            10, 6, 1, // Code section, size in bytes, element count
            4, // Code element 1;  WRONG Size in bytes: should be 3.
            0, // Local count
            0x1, 0xb, // Body: nop, end
    ...
    let invalid_instruction = [
        ...
        &[
            10, 5, 1, // Code section, size in bytes, element count
            3, 0, // Code element size, local count
            0xff, 0xb, // Body: WRONG
        ],
    ]
    .concat();
